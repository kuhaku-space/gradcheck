...
└── lib/                   ← UI 非依存の純粋ロジック（Vitest の対象）
    ...
    ├── studentId.ts       学籍番号からの入学年度推定
    ├── rules.ts           入学年度別の修了要件ルールセット（根拠は docs/requirements.md）
    └── judge.ts           判定ロジック
```
...
- **要件は `rules.ts` のルールセットに集約**: 学籍番号から推定した入学年度で
  適用するルールセットを選ぶ（経過措置対応）。規程改定時は `RULE_SETS` に
  新しい入学年度からの定義を追加するだけでよい。科目区分の解釈は CSV の
